...
The worker makes routine implementation choices, investigates uncertainties and checks results within the packet's delegated decisions. Do not ask the main session to approve every detail. When evidence exposes a requirement conflict, shared design/contract choice or missing authority outside that scope, return the relevant source/revision, observation, failed or rejected hypotheses, acceptance impact and one concrete decision needed. Keep the dependent change pending; continue only independent authorized work.

The main session examines that evidence against the requirement, context, decomposition and hypotheses, resolves the shared decision within its authority or asks the user for the actual missing decision, then sends the decision and affected acceptance back to the execution owner. Uncertainty alone does not trigger a model upgrade, duplicate implementation or ownership transfer. Apply [allocation](policy.md#capability-choice-and-adaptive-allocation) and the existing change-receipt and repair rules; preserve the worker's ownership while it can continue safely.
